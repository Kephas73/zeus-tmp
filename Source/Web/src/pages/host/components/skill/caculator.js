...
export const formatDataForLanguages = (fromDateTime, toDateTime) => {
  ...
  const funcCountTimeWaiting = (language, fromDateTime, toDateTime) => {
    const formatDataForLanguages = filteredRoomChangeLogs.filter((room) => {
      ...
      const isWithinTimeRange = timestampSeconds >= fromDateTime._seconds && timestampSeconds <= toDateTime._seconds;
  ...
  const getCallWaitingAverageWaitingTime = (language, funcGetDate, dateTime) => {
    ...
    const filterWaitingTimeForLanguage = filteredWaitingGuests.filter((e) => e.skillId === language)
  ...
  const funcCountCallWaitingNumberOfSuccessfulConnections = (language, funcGetDate, dateTime) => {
    const filterWaitingTimeForLanguage = filteredWaitingGuests.filter((e) => e.skillId === language)
    const callWaitingNumberOfSuccessfulConnections = filterWaitingTimeForLanguage.reduce((accumulator, item) => {
      const date = funcGetDate(item.start);
      if (funcGetDate(dateTime) === date) {
  ...
  const calculateDataForLanguage = (language) => {
    ...
    const countNumberOfActiveSeats = funcCountNumberOfActiveSeats(language, getYearMonthDay, fromDateTime);
    ...
    const countNumberOfMissedCalls = funcCountNumberOfMissedCalls(callsForLanguage, getYearMonthDay, fromDateTime);
    const countNumberOfBreaks = funcCountNumberOfBreaks(callsForLanguage, getYearMonthDay, fromDateTime);
    const callWaitingAverageWaitingTime = getCallWaitingAverageWaitingTime(language, getYearMonthDay, fromDateTime);
    const callWaitingNumberOfSuccessFulConnections = funcCountCallWaitingNumberOfSuccessfulConnections(getYearMonthDay, fromDateTime)
    const callWaitingNumberOfExits = funcCountCallWaitingNumberOfExits(language, getYearMonthDay, fromDateTime)
    ...
    const callWaitingRate =
      numberOfCallsWaiting / (numberOfIncomingCalls - countNumberOfMissedCalls - countNumberOfBreaks);
    return {
